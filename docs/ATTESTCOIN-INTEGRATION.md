...
- **Mainnet (chainKey 3): verification boundary exercised, credit file unchanged. [MEAS]**
  ...
  while the production `CreditFile` currently ingests the configured Sepolia source. The six
  ...
  One authoritative receiver is a deliberate invariant, not an omission: keeping the
  production credit state single-writer and auditable was worth more than a second ingress
  path added late. A first attempt was correctly rejected with `UnauthorizedSource` because the transaction was
